	...
	<div class="main-body">
		...
		<div class="wrap">
			...
			<?php
				...
				$sql = $mysqli->query("SELECT id_category, nama_category FROM tb_category");
				...
				$query = $mysqli->query("SELECT tb_berita.id_berita, tb_berita.baca, tb_berita.id_wartawan, tb_category.nama_category, tb_category.id_category, tb_berita.judul,DATE_FORMAT(tb_berita.tgl, '%d-%m-%Y') as tgl, SUBSTRING(tb_berita.isi, 5,100) as isi, tb_berita.image FROM tb_berita LEFT JOIN tb_category ON tb_berita.id_category = tb_category.id_category ORDER BY tb_berita.id_berita DESC LIMIT $start, $limit");
			...
			<?php }?>
						...
						$lala = $mysqli->query("select * from tb_berita");
						$rows = mysqli_num_rows($lala);
						$total=ceil($rows/$limit);
						if($id>1)
						{
						echo '<li class="previous"><a href="page-'.($id-1).'">← Older</a>
							</li>';
						}
						if($id!=$total)
						{
						echo '
						  <li class="next"><a href="page-'.($id+1).'">News →</a>
							</li>';
						}
						if($id==$total)
						{
