	...
	<div class="main-body">
		...
		<div class="wrap">
			...
			<?php
				...
				$sql = $mysqli->query("SELECT nama_category FROM tb_category WHERE id_category = '$id_category'");
				$title = $sql->fetch_assoc();
			?>
			<div class="articles">
			<header>
				<h3 class="title-head"><?php echo $title['nama_category']?></h3>
			</header>
			...
			<?php
				...
				$query = $mysqli->query("SELECT tb_berita.id_berita, tb_berita.baca, tb_berita.id_wartawan, tb_category.nama_category, tb_category.id_category, tb_berita.judul,DATE_FORMAT(tb_berita.tgl, '%d-%m-%Y') as tgl, SUBSTRING(tb_berita.isi, 2,100) as isi, tb_berita.image FROM tb_berita LEFT JOIN tb_category ON tb_category.id_category = tb_berita.id_category WHERE tb_category.id_category = '$id_category' ORDER BY tb_berita.id_berita DESC LIMIT $start, $limit ");
			...
			<?php }?>
						...
						<ul class="pager">
						  <li class="next"><a href="page-2">News →</a>
							</li>
                        </li>
                    </ul>
				</div>
					...
					<form>
					 ...
					 <div class="popular-news">
						...
						<?php
							...
							$url_link = "post-".$pop['id_berita']."-".$Judul."";
						?>
